...
It does not yet determine what function a completed trajectory may perform within another context.
...
It does not authorize arbitrary micro-description, decomposition of the PMS operators themselves, or unmarked transition into cross-level functional projection.
...
high Ω relevance
≠
Ω exists without its dependencies
...
invalid_projection:
  error_type:
  source_object:
  claimed_target_function:
  missing_context:
  missing_source_trace:
  type_integrity_failure:
  level_mixing:
  scope_inflation:
...
It does not authorize origin-type replacement, arbitrary higher-level labeling, semantic inflation through analogy, new PMS primitives, or additional analytical authority.
...
+
unchanged or source-independent result
=
...
It prevents additional resolution from becoming distinction without praxeological purchase, prevents higher composition and projection from becoming abstraction without traceable load, and preserves the possibility of claim reduction, mandatory stop, failure, and non-capture.

STRATA remains valid only where its transformations are both discriminating and reconstructively anchored.
...
```text
PATH
temporally composes configurations and transitions.

SUB
decomposes provisionally compressed occurrences and composites.

RETYPE
projects origin-typed objects as bounded functions in declared contexts.
...
Decomposition reconstructs selectively.

Projection foregrounds a bounded function.
...
Counterfactual Sensitivity provides an additional load test:
...
```text
PMS Base
provides the operator grammar.

PATH
forms temporally structured objects.

SUB
opens provisionally compressed objects.

RETYPE
specifies bounded cross-level functions.

LIMITS
controls relevance, traceability, continuity, loss, stop, and non-capture.
```

STRATA therefore does not create a new theoretical authority above PMS.
