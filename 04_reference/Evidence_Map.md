...
**Repository role:** source, support, loss, gap, and evidence-boundary routing; not an independent theory or authority source 
...
This map records what kind of source relation would be needed to support, limit, contradict, rival, or leave open a tested STRATA claim. It routes current control provenance, realized formal-model and smoke-test handoffs, and case, calibration, and external-warrant evidence without treating navigation, citation, formal consistency, or repository volume as substantive proof.
...
- distinguish formal conformance, active case pressure, calibration limits, and application-specific external warrant explicitly.
...
- **authority provenance:** why an artifact controls PMS or STRATA method language and routing;
...
| `00_source/PMS-STRATA_Structure.md` | architecture and chapter blueprint | not evidence that chapter claims are true |
| `01_blocks/*` | canonical corpus prose | canonical prose still requires case, source, and claim discipline |
...
| `03_cases/*` | active positive, negative, confusion, stop, and non-capture tests | cases test rules; they do not define theory |
...
An artifact can be authoritative for terminology or corpus architecture while providing no direct support for a concrete historical, functional, or causal claim.
...
The roles below are controlled routing categories, not a closed formal enum. Cases may reveal a needed local wording, but any genuinely new system-wide role requires Reference Kernel revision before formalization.
...
Case carriers in this section are testing pressure, not universal validation or independent theory sources.
...
## 25. Active Evidence and Control Carriers

| Object | Active carrier | Evidential role | Boundary |
| --- | --- | --- | --- |
| PMS Base grammar | `PMS.yaml` | operator inventory, dependencies, and Base guardrails | not empirical domain evidence |
| STRATA architecture | `00_source/PMS-STRATA_Structure.md` | chapter and artifact architecture | not canonical prose by itself |
| canonical method claims | `01_blocks/*` | governing STRATA prose | not application-specific truth evidence |
| operationalization | `02_appendices/*` | notation, templates, tests, and audit forms | no competing definitions |
| case pressure | `03_cases/*` | positive, negative, confusion, chain, Stop, and Non-Capture tests | no universal validation from cases alone |
| Reference control | `04_reference/*` | terminology, ownership, support, routing, and audit consistency | not substantive source evidence |
| binding controls | `05_minified/*` | compressed constraints and contracts | no authority above canonical prose |
| formal consistency | `07_model/*` | schema, registry, rule, and record consistency | machine validity is not semantic or empirical validity |
| external warrant | application-specific declared sources | empirical, causal, legal, normative, or domain warrant where required | never inherited from STRATA structure |

## 26. Open Evidence Registry

| Open bounded duty | Current status | Required handling |
| --- | --- | --- |
| Chapter 16 same-source PATH boundary comparison | partial | retain `partially_admissible` coverage until the exact comparison set exists |
| Chapter 25 same-source resolution comparison | partial | retain `partially_admissible` coverage until one shared source supports gain, neutrality, drift, escape, and Stop pressure |
| external empirical, causal, legal, or normative warrant | application-dependent | declare externally; never infer from formal consistency or structural fit |
| rival reconstructions and calibration | record-dependent | disclose relevant rivals, comparison basis, and unresolved uncertainty |
| Reference Freeze evidence | open bounded duty | verify ownership, carrier continuity, links, duplicates, and unresolved claims without turning absence into completion |
