...
**Authority basis:** `PMS.yaml`, `00_source/PMS-STRATA_Structure.md`, `05_minified/*`, the provisionally locked canonical Foundations Chapters 0–8 in `01_blocks/01_foundations.md`, the provisionally controlled Reference Kernel, and `../_workfiles/chapter_preparation/Chapter_1_Preparation_Record.md` through `../_workfiles/chapter_preparation/Chapter_8_Preparation_Record.md` as non-theory production controls
...
Primary production-control route: `../_workfiles/chapter_preparation/Chapter_4_Preparation_Record.md`. Canonical operation-grammar ownership is now Chapter 4 Sections 4.1–4.10; integrated lock remains pending WP4.
...
Primary production-control route: `../_workfiles/chapter_preparation/Chapter_5_Preparation_Record.md`. Canonical claim ownership remains future Chapter 5 prose; Chapter 47 later systematizes failure and audit relations.
...
Production control: [`Chapter 8 Preparation Record`](../_workfiles/chapter_preparation/Chapter_8_Preparation_Record.md). These claim forms do not add Output Classes or replace Chapter 6 and Chapter 7 routing.
...
Prepared case duties `C9-SCOPE-01` through `C9-NC-01` remain non-evidence production assignments. Canonical drafting is pending. Production control: [`Chapter 9 Preparation Record`](../_workfiles/chapter_preparation/Chapter_9_Preparation_Record.md).
...
Prepared cases `C10-DEF-01` through `C10-NC-01` are production obligations, not evidence. Production control: [`Chapter 10 Preparation Record`](../_workfiles/chapter_preparation/Chapter_10_Preparation_Record.md).
...
Canonical route: [`Chapter 10 §§10.1–10.6`](../01_blocks/02_part_i_path.md#chapter-10-path). The [`Preparation Record`](../_workfiles/chapter_preparation/Chapter_10_Preparation_Record.md) remains production history.
...
Distinct claims require distinct record segments and may receive distinct canonical Output Classes. Class stacking on one undifferentiated claim remains prohibited. Preparation control: [`../_workfiles/chapter_preparation/Chapter_20_Preparation_Record.md`](../_workfiles/chapter_preparation/Chapter_20_Preparation_Record.md).
...
## Chapter 41 WP1 Recursive-Continuation Claim

| Claim form | Required burden | What it cannot establish automatically |
|---|---|---|
| another operation is technically available | identifiable source object and valid operation applicability | necessity or admissibility |
| another operation is necessary | current limitation plus fresh expected praxeological difference | substantive passage |
| another operation is admissible | separate Record, source support, Loss, continuity, Admissibility Band, and result | repair of a prior claim |
| an operation chain is admissible | passage of each occurrence plus admissible chain relation and preserved prior dispositions | authority increase or local-result override |

Primary site: [§41.2](../01_blocks/05_part_iv_limits.md#41-2-recursive-availability).

## Chapter 41 WP2 — Claim Relocation and Authority Ceiling

A changed target function, frame, granularity, relative level, composite, or reference object creates a new claim burden. It does not automatically answer the prior claim. The record must preserve the prior claim identifier and disposition, state the new claim type, and declare the relation between them.

No structural claim produced by STRATA alone supplies normative legitimacy, recommendation, sanction, governance authority, diagnosis, or person rank. Primary sites: [§41.6](../01_blocks/05_part_iv_limits.md#41-6-vertical-authority-drift) and [§41.7](../01_blocks/05_part_iv_limits.md#41-7-analytical-self-immunization).

## Chapter 41 WP3 — Constitutive-LIMITS Claim Boundary

| Claim position | Supported claim | Prohibited inference |
|---|---|---|
| operation available | a further analytical question can be formulated | operation is necessary or admissible |
| operation locally admissible | the specific occurrence passes within its bounded claim | later operation or chain automatically passes |
| integrated chain admissible | local occurrences and handoffs pass under preserved burdens | prior failure is erased or external authority is gained |
| Chapter-41 rationale complete | method-level need for constitutive LIMITS is established | Q1–Q3 are adjudicated or Part IV is locked |
...
## Chapter 41 WP4 and Chapter 42 Claim Separation

| Claim | Permitted status | Prohibited inference |
|---|---|---|
| Chapter-41 method-rationale completion | `admissible_but_provisional` | Part-IV lock or pressure-object passage |
| analytical-level claim | locally coordinate- and source-bound | independently existing real layer |
| decomposition claim | finer reconstruction for a declared question | final constituent or fundamental essence |
| composition claim | selective composite for a bounded claim | ultimate totality |
| projection claim | contextual function with origin type preserved | ontological promotion |
| operator-grammar claim | reconstruction grammar | world inventory or universal causal architecture |
| corrective formulation | `Within frame C and granularity g, X can be reconstructed through Y for claim Q` | metaphysical reduction |

Primary route: [Chapter 42 Preparation Record](../_workfiles/chapter_preparation/Chapter_42_Preparation_Record.md).

## Chapter 42 WP1 — Bounded Level Claims

A level claim must identify the reference object, source and target positions, Frame, granularity relation, comparison or transformation context, Source Scope, temporal scope where relevant, and Claim Scope. A level predicate without recoverable coordinates requires expansion or claim reduction.

No level claim creates a truth rank, causal priority, person status, recommendation, sanction, governance competence, or application authority. Primary site: [§42.1](../01_blocks/05_part_iv_limits.md#42-1-analytical-levels).

## Chapter 42 WP2 Claim-Ceiling Note

A component, composite, or target-function claim may be strongly supported within its declared scope while remaining barred from final-constituent, totality, or ontological-promotion language. Claim strength does not remove Frame, source, granularity, Loss, or context dependence.


## Chapter 42 WP3 Claim-Type Boundary

| Formulation | Claim type within STRATA | Boundary |
|---|---|---|
| PMS occurrence typed under Frame and source support | bounded structural reconstruction claim | not a world-constituent claim |
| relative level or granularity statement | local coordinate claim | not a real-layer claim |
| corrective bounded formulation | repaired claim syntax requiring later substantive testing | not an admissibility result |
| reality consists of Δ–Ψ layers | ontology claim outside STRATA warrant | must be withdrawn, separated, or marked outside method scope |
| relevant structure exceeds current grammar | capture-limit claim | does not imply unreality or unknowability |

Primary sites: [§42.7](../01_blocks/05_part_iv_limits.md#42-7-operator-grammar-versus-world-structure) and [§42.9](../01_blocks/05_part_iv_limits.md#42-9-corrective-formulation).

## Chapter 42 Lock and Chapter 43 Comparison Claims

Chapter 43 distinguishes at least four claim objects:

- a candidate reconstruction claim at one granularity;
- an alternative candidate claim at another granularity;
- a wider-composition claim at a declared compositional scope;
- a comparative-performance claim relating two or more candidates.

```text
candidate claim warranted
...
micro-support claim
≠ macro-entailment claim

chapter-method lock
≠ pressure-object result
```

Every comparison claim requires declared comparison coordinates and must preserve each candidate's prior disposition and Loss.


## Chapter 43 WP1 Claim-Type Note

Comparative claims must state the question and analytical coordinates for which a reconstruction performs. A local preference claim such as `RC performs better for QC` does not generalize into a universal coarse-granularity claim. WP1 establishes the claim form but adjudicates none of `RC`, `RF`, or `RH`.

```text
claim-relative performance
≠ universal granularity ranking
```

## Chapter 43 WP2 Claim-Type Note

`RC`, `RF`, and `RH` remain separate reconstruction claims. Co-validity is a relation among separately warranted claims, not a merged claim type. A local preference must state the question and comparison coordinates for which it holds; it does not raise another candidate's Claim Ceiling or erase its distinct scope.

Primary site: [§43.6](../01_blocks/05_part_iv_limits.md#43-6-coarse-and-fine-co-validity).


## Chapter 43 WP3 — Reduction and Comparative Claim Controls

A reduction, derivation, replacement, or comparative-superiority statement is a separate testable claim. It must identify the compared claims, common reference or explicit relation, Frame, granularities, source basis, constitutive relation, PraxisPurchase, TraceableLoad, Loss, alternatives, and bounded Claim Scope.

```text
source-supported local claim
≠ warranted macroclaim

warranted macroclaim
≠ exhaustive local claim set
```

Re-entry through new sources, Frame, granularity, compositional scope, or target function creates a new claim and does not erase an earlier failure, Stop, underdetermination, or Non-Capture result.

Primary sites: [§43.7](../01_blocks/05_part_iv_limits.md#43-7-no-automatic-reduction) and [§43.9](../01_blocks/05_part_iv_limits.md#43-9-granularity-humility).

## Chapter 43 Lock and Chapter 44 Claim Boundary

Chapter 44 requires separation of the proposed distinction, source-support claim, changed-reconstruction claim, changed-claim statement, lower-bound finding, Stop finding, and canonical Output Class. Relevance remains claim-relative; a distinction may be relevant for one claim, neutral for another, and source-limited for a third.


## Chapter 44 WP1 Claim-Type Note

A relevance claim must identify the proposed distinction, prior claim, alleged revised claim, affected praxis dimension, source basis, analytical coordinates, alternatives, and Claim Scope. A shift of Frame, granularity, reference object, target function, source basis, or question creates a new testable claim and does not erase the earlier result.

```text
D relevant for Q1
≠ D relevant for Q2 automatically
```

## Chapter 44 WP2 — Changed-Claim Comparison

A lower-bound claim comparison must retain:

| Position | Required content |
| --- | --- |
| `Q` | exact prior defensible claim |
| `D` | exact added distinction |
| `Q′` | exact candidate changed claim |
| relation | source-supported constitutive `D → Q′` relation |
| dimension | affected praxis dimension |
| boundary | alternatives, Loss, Claim Scope, and Claim Ceiling |

A narrower, conditional, separated, defeated, or stopped `Q′` can be a possible changed reconstruction. A new label or equivalent reformulation is not sufficient.
## Chapter 44 WP3 — Lower-Bound Claim and Disposition Types

| Local finding | Required claim content | Not equivalent to |
| --- | --- | --- |
| supported negative finding | `D` tested and shown not load-bearing for exact `Q` | absent evidence or universal irrelevance |
| resolution-neutral candidate | supported `D`, continuous reference, unchanged warranted `Q`, no hidden load-bearing Loss | failed transformation, source limitation, or Non-Capture |
| lower Stop trigger | continued differentiation lacks warrant for the current route | permanent closure of the object |
| re-entry claim | materially new source/question/Frame/granularity/reference/claim/alternative/operation with prior disposition retained | repetition or relabeling |

These are local claim and disposition forms. Final canonical Output-Class selection remains one class per delimited routed claim after all applicable checks.


## Chapter 44–45 claim-axis handoff

The lower-bound question—whether `D` changes `Q`—remains distinct from the upper-bound question—whether the target claim retains constitutive source load. A target claim may possess PraxisPurchase and still fail TraceableLoad; it may retain trace while requiring Claim Reduction; or current sources may leave the dependency underdetermined. Claim status, support status, local boundary finding, Stop, and canonical Output Class remain separate axes.

## Chapter 45 WP1 — Source-support and traceability claim separation

| Claim | Required burden | Does not establish |
|---|---|---|
| Source-location claim | identifiable evidence location | Constitutive Source Trace |
| Structural-mapping claim | source feature/relation mapped to target element | unique target warrant |
| Functional-dependence claim | bounded reason the target depends on the trace | universal causality |
| Temporal-load claim | constitutive order, duration, recurrence, or formation retained | exhaustive history |
| TraceableLoad claim | combined reconstructible source-load account | complete admissibility or authority |

`T`, `TL`, `TE`, and `TS` remain separate unadjudicated claims.

## Chapter 45 WP2 claim controls

A Source-Target Dependency claim must identify a load-bearing source feature or relation, a warranted modification, the affected target-claim element, and the expected claim effect. It remains bounded by source, context, origin type, continuity, Loss, Claim Scope, and Claim Ceiling and does not establish causal truth, universal necessity, or authority.

## Chapter 45 WP3 claim controls

| Claim relation | Required control | Prohibited shortcut |
|---|---|---|
| stable target across rival sources | identify preserved invariant or disclose elasticity/underdetermination | stability proves robustness |
| compressed target claim | preserve load-bearing trace and five-part Loss | exhaustive reproduction required |
| strong claim exceeds source trace | state unsupported excess and stop or reduce | rhetorical qualification repairs claim |
| residual bounded claim | explicit restatement, lineage, and retest | inherit positive status from source trace alone |
| local upper-bound finding | retain as local gate result until integrated mapping | treat as automatic Output Class |

## Chapter 45–46 claim-axis handoff

A Source-Target Dependency claim states why a target depends on declared source load. A Counterfactual Sensitivity claim tests that dependency through source-bounded relevant modification. Feature status, modification status, target-response status, sensitivity value, support status, claim effect, Stop, and canonical Output Class remain separate axes. Untestability is a source condition, not truth, falsity, robustness, or immunity.

## Chapter 46 WP1 — Counterfactual claim separation

| Claim | Required burden | Does not establish |
|---|---|---|
| Baseline claim | identified source object, feature, coordinates, target, and scope | sensitivity result |
| Modification-relevance claim | source-bounded change to declared load | actual alternate outcome |
| Expected-response claim | exact target element and claim effect expected to change | causal necessity |
| Source-discipline claim | variation and response stay within available warrant | full admissibility |

`F`, `M`, `R`, and `U` remain separate claims and must not be collapsed.

## Chapter 46 WP2 — Sensitivity claim forms

| Local finding | Claim-relative meaning | Does not establish |
|---|---|---|
| `strongly_sensitive` | central target claim materially changes under a supported relevant modification | causality or automatic failure |
| `partially_sensitive` | bounded claim element changes while an explicit residual claim remains | automatic `partially_admissible` or bounded passage |
| `weakly_sensitive` | detectable secondary target change with central claim substantially stable | irrelevance or near-insensitivity |
| `insensitive` | expected target response absent under adequate test conditions | robustness, elasticity, or failure automatically |
| `underdetermined` | multiple materially different responses remain plausible | permission to choose one |
| `untestable` | current sources cannot support the modification or response test | truth, falsity, robustness, or immunity |

The full claim-effect mapping remains incomplete until Chapter 46 WP3 and later LIMITS gates.

## Chapter 46 WP3 — bounded claim effects

A sensitivity result is compared with the dependence burden actually asserted by the claim. Weak sensitivity may defeat a necessity claim while remaining adequate for a limited-modulation claim. New bounded or successor claims require separate records and preserve prior dispositions.
...
No sensitivity value mechanically fixes claim support, confidence, reduction, Stop, or canonical Output Class.

## Chapter 46–47 continuity claim separation

Reference-identity, origin-type, bounded-function, temporal-load, and chain-continuity claims remain separate. A name-preservation claim does not establish historical reference; preserved type does not establish functional continuity; preserved function does not establish temporal continuity; and a Chapter-level method lock does not adjudicate any pressure-object claim.

## Chapter 47 WP1 — reference-continuity claim controls

| Claim form | Minimum burden | Insufficient substitute | WP1 status |
|---|---|---|---|
| Reference Continuity claim | same referent, process, relation bundle, preservation criterion, alternatives, Loss, scope | name or label persistence | method established; pressure candidate unadjudicated |
| Reference Discontinuity claim | exact crossed preservation criterion and replaced/severed identity-bearing relation | mere change or relabeling | method established; pressure candidate unadjudicated |
| Type-continuity statement | canonical `TypeIntegrity` view | new continuity Rule | architectural boundary fixed |
| Mixed continuity statement | separate dimension findings | one binary continuity status | permitted; integrated result deferred |

## Chapter 47 WP2 claim separation

| Claim | Required declaration | Must not be inferred from |
|---|---|---|
| type-preservation claim | source origin type, operation, target typing, prohibited collapse check | label persistence or target-function wording |
| type-revision claim | prior typing, revision basis, corrected bounded typing | later analytical convenience |
| Functional Continuity claim | source features, constitutive relation, target function, context, validity scope | resemblance or thematic fit |
| Functional Discontinuity claim | exact failed source-function dependency | generic weakness or context change alone |
| analogy claim | declared resemblance and limits | valid `PROJECT_AS` identity |

## Chapter 47 WP3 — continuity claim effects

A continuity claim must state the dimension, Preservation Criterion, source basis, limits, Loss, and Claim Scope. Temporal claims identify the exact order, duration, recurrence, turning point, Non-Event, irreversibility, or historical load required. Mixed findings may narrow reference, context, time, function, or strength; each successor formulation is a new testable claim and does not erase the prior disposition.

## Chapter 47 WP4 and Chapter 48 Preparation — selection and Loss claims

A Loss claim must identify the transformation occurrence, selected coordinates, affected source or target structure, one or more of the five canonical Loss fields, the constitutive relation to the claim, alternatives, Claim Scope, and whether the loss was inherited or introduced. A successor selection or reopening is a new testable claim and does not erase the earlier disposition.

## Chapter 48 WP1 — selection and Loss claim burdens

A selection claim must declare the operation, analytical coordinates, source subset, constitutive relations, alternatives, and Claim Scope. A preservation claim identifies the surviving structure and its return path. A compression claim names reduced detail, retained load, and reopening conditions. An exclusion claim names the omitted structure, reason, scope, source status, and possible claim effect. A successor selection remains a new testable claim and does not erase earlier Loss or disposition.

## Chapter 48 WP2 claim additions

A Reconstruction-Selection claim must declare selected coordinates, materially available alternatives, the reason for selection, and the claim effect of restoring a rejected alternative. An irrecoverability claim must identify the unrecoverable structure, loss location, attribution status, and affected Claim Scope. An uncertainty claim must state the live possibilities and missing discriminator without converting source limitation into precision.

## Chapter 48 WP3 claim additions

A Compression-Debt claim must identify the earlier transformation, prior bounded Claim Scope, compressed structure, later trigger, newly load-bearing relation, and available reopening route. A Hidden-Loss claim must identify the mismatch between represented and source-supported Loss status and the way that mismatch strengthens the claim. A calibrated successor claim must state the surviving load-bearing structure and preserve the disposition of the earlier claim.

## Chapter 48 WP4 and Chapter 49 source-bounded claim preparation

A source-bounded claim must declare source type, direct support, inferred structure, missing structure, inferential distance, Loss profile, alternatives, Source Ceiling status, calibration threshold or openness, Claim Scope, and Claim Ceiling. A successor claim based on new evidence or a changed threshold is a new testable claim and preserves the earlier disposition.

## Chapter 49 WP1 — source-bounded claim burdens

A source-bounded claim must identify the source basis, Source Scope, direct support, bridging inferences, missing structures, alternatives, Loss, analytical coordinates, and maximum supported claim reach. A Non-Event claim additionally requires a warranted expectation frame, bounded realization condition, source-supported non-realization, and praxis-relevant difference. A distant claim must expose every material bridge and contract where a bridge lacks independent support.

## Chapter 49 WP2 — calibrated claim burdens

A calibrated claim declares stable comparison coordinates, relevant differences, a visible local threshold or range, and at least one possible threshold-failing case. A calibration-open claim additionally states competing threshold candidates, affected cases, missing discriminator, and the bounded structural claim surviving all defensible calibrations.

## Chapter 49 WP3 — ceiling-bounded and calibration-open claims

A ceiling-bounded claim states the strongest source-supported structure below the identified unsupported next distinction. A calibration-open claim states the supported form, live threshold candidates or range, affected classifications, missing discriminator, and invariant bounded claim. Neither may inherit precision from schema validity, model completion, or operator labels.

## Chapter 49 WP4 and Chapter 50 Preparation — original and successor claims

An original claim and a successor reconstruction remain separate claim objects. The successor must identify its changed coordinates or operation, additional source support, independent test, and relation to the original objection. Success never retroactively repairs the original disposition.

## Chapter 50 original/successor claim control

Original and successor claims require separate identifiers whenever a material coordinate, operation occurrence, composition, source basis, or target function changes. A successor may be narrower, differently framed, or independently projected; it may not inherit repair status for the original claim. Earlier objections, Loss, and dispositions remain linked and visible.

## Chapter 51 claim and re-entry control

Every Stop record preserves claim before Stop, any retained or reduced claim, the failed or unsupported component, current support status, unresolved remainder, and future re-entry condition. Claim Reduction changes claim content or scope; support downgrade changes evidential status; `resolution_neutral` records a tested absence of resolution gain. Re-entry creates a new testable claim or controlled revised record.

## Chapter 52 capture-claim control

Every Non-Capture claim declares the delimited capture object, attempted operations and coordinates, captured and uncaptured structure, limiting condition, source and Loss basis, rivals, claim effect, prior Failure or Stop, and re-entry conditions. A broader whole-claim Non-Capture result may coexist with narrower captured claims; each remains independently testable.

## Chapter 53 final claim control

The final integrated claim records direct, inferred, provisional, reduced, unresolved, and excluded components. The strongest supported claim is bounded by Source, Calibration, Claim, and Authority Ceilings. A broader failed claim may coexist with a narrower retained claim, but each remains separately identified and testable.

## Chapter 54 integrated claim boundary

An integrated-model claim may summarize how PMS Base, Parts, operations, records, and LIMITS relate. It may not strengthen any upstream empirical, causal, semantic, ontological, person-level, normative, or application claim. Every chain claim preserves local claim scopes, source limits, Loss, and prior dispositions.

## Chapter 55 capability-claim boundary

A capability claim states that STRATA supplies an inspectable method for a defined reconstruction burden. It is not an empirical domain finding, causal claim, person claim, prediction, policy claim, intervention warrant, or authority grant. Capability claims inherit all source, coordinate, Loss, Claim Ceiling, and Authority Ceiling limits.

## Chapter 56 prohibited-claim registry

Chapter 56 owns the complete terminal registry of claims not warranted by STRATA: Base revision, superior layer, objective stratum ontology, final constituents, totality, scale privilege, automatic retyping, losslessness, unlimited recursion, universal thresholds, causal proof, person typing, automatic cross-domain validity, counterexample immunity, application authority, and guaranteed full capture. These are method boundaries, not universal impossibility claims.

## Chapter 57 final-claim boundary

The strongest terminal claim is methodological and bounded: declared vertical movements can be made explicit, typed, source-bounded, loss-accounted, limited, and criticizable. It is not an empirical-completion, universal-capture, causal, person-level, normative, application-authority, or release-readiness claim.



## RETYPE Lock Package 1 synchronization

Registers one case-local bounded trajectory-to-frame-function claim without adding a canonical claim type.

## RETYPE Lock Package 2 — failed contextual-function claim

`C40-N3` separates a nominal metadata statement from a contextual-function claim. “The registry assigns label L” may be retained as a label fact; “source object X performs function L” fails where constitutive Source Trace, PraxisPurchase, functional continuity, and sensitivity are absent. Changing to the label fact is a separate claim, not automatic Claim Reduction of the failed function.

## RETYPE Lock Package 3 — analogy and projection claim segmentation

`C40-X6` separates two claim segments: (1) a bounded structural-analogy claim retained under `analogy_only`; and (2) a contextual target-function candidate that remains unretained because Semantic Preservation and constitutive Source-Function Dependency are not established. A later projection is a new transformation and new testable claim.
